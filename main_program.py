...
def read_technicians_data():
    technicians = []
    try:
        with open("techicians_data.json", encoding="utf-8") as f:
...
def store_technicians_data(technicians):
    ...
    with open("techicians_data.json", "w", encoding="utf-8") as f:
...
def main():
    ''' ΤΟ ΟΝΟΜΑ ΤΟΥ ΑΡΧΕΙΟΥ ΝΑ ΠΕΡΝΑΕΙ ΩΣ ΠΑΡΑΜΕΤΡΟΣ ΣΤΗ ΣΥΝΈΡΤΗΣΗ '''
    technicians = read_technicians_data()
    program = Program(technicians)
    ...
    if program.has_technicians_out_of_order():
        for i in range(len(program.technicians_out_of_order)):
            ...
            if technician.is_available(first_guard):
    ...
    for tech in program.sequence:
        ...
        while not technician.is_available(next_free_day):
            ...
            if next_guard <= program.days_of_month:
                ...
                program.add_technician_to_next_month(technician.tech_id, next_free_day, next_guard - program.days_of_month, reason)
        ...
        if program.day_is_empty(day):
            ...
            while not technician.is_available(day):
                ...
                if next_guard < program.days_of_month:
                    ...
                    program.add_technician_to_next_month(technician.tech_id, from_day, next_guard - program.days_of_month + 1, reason)
                # Reduce the number of available technicians by one
                program.active_technicians -= 1
